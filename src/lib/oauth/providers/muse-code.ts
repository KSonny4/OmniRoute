...
export const museCode = {
  ...
  requestDeviceCode: async (config: MuseCodeOAuthConfig): Promise<DeviceCodeResponse> => {
    ...
    if (!response.ok || !deviceCode || !userCode || !verificationUri) {
      ...
      throw new Error(`Meta Muse device authorization failed: ${reason}`);
    }
    ...
    return {
      ...
      expires_in: Number.isFinite(expiresIn) && expiresIn > 0 ? expiresIn : 600,
  ...
  postExchange: async (tokens: DeviceTokenPayload) => {
    if (!tokens.access_token) throw new Error("Meta Muse OAuth response omitted access_token");
